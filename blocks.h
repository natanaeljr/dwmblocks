...
{"", R"(nvidia-smi --query-gpu="utilization.gpu,temperature.gpu,fan.speed" --format=csv,noheader | tr -d ',' | awk '{printf "^b#61afef^^c#1e222a^%02d%%^d^ %02d糖\n",$1,$3}')", 3, 6},
{"", R"((cpu-percent; cpu-speed; sensors-cached | awk '/CPU_FAN/{print $2} /CPU_TEMP/{print $2}' | tr -d '+°C') | xargs | awk '{printf "^c#61afef^%02d%% ^b#61afef^^c#1e222a^%.1fGHz^d^ %02d糖 %d ^d^\n",$1,$2,$4,$3}')", 2, 5},
