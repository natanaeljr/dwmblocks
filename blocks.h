...
{"", R"(nvidia-smi --query-gpu="utilization.gpu,temperature.gpu,memory.used,fan.speed" --format=csv,noheader | tr -d ',' | awk '{printf "[%02d %02d°C %d%s %02d]\n",$1,$3,$4,$5,$6}')", 2, 6},
{"", R"((cpu-percent; cpu-speed; sensors-cached | awk '/CPU_FAN/{print $2} /CPU_TEMP/{print $2}' | tr -d '+°C') | xargs | awk '{printf "[%02d %.1fGHz %02d°C %d]\n",$1,$2,$4,$3}')", 2, 5},
{"", R"(sensors-cached | awk '/SYS_FAN/ {print $2}' | xargs printf "[%d %d]\n")", 3, 4},
{"",R"(sensors-cached | awk '/_TEMP/ {print $2}' | tr -d '+°C' | xargs | awk '{printf "[%d %d %d %d°C]\n",$1,$2,$5,$6}')", 3, 3},
